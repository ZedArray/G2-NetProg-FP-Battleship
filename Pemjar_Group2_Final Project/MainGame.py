...
BG = pygame.transform.scale(pygame.image.load("ui/image2.png"), (1280, 720))
BG2 = pygame.transform.scale(pygame.image.load("ui/image3.png"), (1280, 720))
HomePage = pygame.transform.scale(pygame.image.load("ui/homepage.png"), (1280, 720))
enterName = pygame.transform.scale(pygame.image.load("ui/playerselect.png"), (1280, 720))
...
# stores own gridmap data whether there is a ship or not
shipLocs = [0] * 64

# count of how many ships can be placed
shipLeft = [1, 2, 1]

# stores location of ships and how many cells
shipWholes = []

# stores how many of your ship has been sunk
sunkShip = 0

# which state of the screen it currently is:
# 0 homepage, 1 player select, 2 ship placement, 3 game board, 4 winscreen, 5 losescreen
states = 0

# stores the amount of ships you've sunk
hitandSunk = 0

# stores which player you are
whichPlayer = 0
    ...
    while run:
        match states:
            ...
            case 1: # enter name layout
                p1button = pygame.Rect(348, 270, 928-348, 335-270)
                p2button = pygame.Rect(351, 368, 930-351, 431-368)
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if pygame.mouse.get_pressed()[0]:
                            print(pygame.mouse.get_pos())
                            if p1button.collidepoint(pygame.mouse.get_pos()):
                                whichPlayer = 1
                                turn = True
                                states = 2
                            if p2button.collidepoint(pygame.mouse.get_pos()):
                                whichPlayer = 2
                                turn = False
                                states = 2
            ...
            case 3:
                for event in pygame.event.get():
                    ...
                    if not turn:
                        ...
                        receivedData = pc.get_data()
                        if receivedData == []:
                            print(receivedData)
                            continue
                        targeted = -1
                        targeted = int(receivedData[0])
                        print(targeted)
                        pc.pop_data()
                        ...
                        # send a signal of who's sending the signal
                        pc.send_data(str(whichPlayer))
                        ...
                        if pygame.mouse.get_pressed()[0]:
                            ...
                            for i in range(64):
                                if targetGrid[i][0].collidepoint(pygame.mouse.get_pos()):
                                    if targetGrid[i][1] == 2 or targetGrid[i][1] == 3:
                                        continue
                                    # send the cell location to check
                                    pc.send_data(str(i))
                                    pc.send_data(str(whichPlayer))

                                    # wait for a signal whether hit or no hit
                                    receivedSignal = pc.get_data()
                                    while receivedSignal == []:
                                        receivedSignal = pc.get_data()

                                    returnSignal = int(receivedSignal[0])
                                    pc.pop_data()
                                    ...
                                    # if hit
                                        ...
                                        targetGrid[i][1] = 3
                                    # if hit and sunk a ship
                                    elif returnSignal == 90:
                                        targetGrid[i][1] = 3
                                        hitandSunk += 1
                                        print("sunk")
                                    
                                    # check how many ships you've sunk
                                    if hitandSunk == 4:
                                        states = 4
